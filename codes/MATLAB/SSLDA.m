...
%%Rho = (BB+deldel*ones(p, p))/(1+deldel);
...
for ki=1:K
    ...
    locix=round(1+(ki-1)*n1/K):round(n1/K*ki);
    x1=x(locix,:);
    %x1=x(loci,:);
    x0=x;
    %x(loci,:)=[];
    ...
    lociy=round(1+(ki-1)*n2/K):round(n2/K*ki);
    %y1=y(loci,:);
    y1=y(lociy,:);
    y0=y;
    %y(loci,:)=[];
    ...
    for i=1:(n1/K)
    cv(ei)=cv(ei)+ldacss(x0(locix(i),:),x2,y2,epsilon);
    end

    for i=1:(n2/K)
    %cv(ei)=cv(ei)+1-ldacss(y0(locix(i),:),x2,y2,epsilon);
    cv(ei)=cv(ei)+1-ldacss(y0(lociy(i),:),x2,y2,epsilon);
    end
end
...
%beta = clime(x0,st, smxy', epsilon, pdtol, pdmaxiter, cgtol, cgmaxiter)/p;
beta = clime(x0,st, smxy', epsilon, pdtol, pdmaxiter, cgtol, cgmaxiter)/trace(Sigma);

%z=mvnrnd(mu1,Sigma,n1);
%z=contaminate_data(z,0.1);
%z=mvt(mu1',Sigma,2,n1); %自由度为5的t分布
...
%z=contaminate_data(z,0.1);
%z=mvt(mu2',Sigma,2,n2); %自由度为5的t分布
...
ce=1-(n1*mean(result1)+n2*mean(result2))/n;
end
